...
// v12: the panel gained ONE polarity switch (cfg.faceTheme), and a tile with
// an explicit `theme` outranks it — which is right for a deliberate
// odd-one-out and wrong for the tiles that carry `theme: 'inverted'` only
// because the old per-tile checkbox wrote the value it was already showing.
// Those tiles would have ignored the switch, which is precisely the
// visit-every-widget chore the switch exists to remove.
//
// `'inverted'` is what an unset theme already means, so removing it cannot
// change how anything renders — it only lets the panel switch reach the tile.
//
// `'normal'` is NOT a deliberate value either, which is the correction that
// cost a live check: the old checkbox stored the explicit value BOTH ways, so
// unticking it on every tile is exactly how somebody ran a black-on-white
// panel — and the production face was doing precisely that. Stripping those
// blindly would have flipped a working panel to its opposite on the next
// render. So `'normal'` is only lifted when the WHOLE config agrees
// (liftUniformNormalTheme below): the screen-wide intent becomes
// `faceTheme: 'light'`, the per-tile copies go, and the render is unchanged.
// A mixed config keeps every explicit value, because there the odd one out is
// real.
//
// Idempotent by construction: after the first pass there is no 'inverted' to
// remove, and a tile pinned deliberately after the config has been stamped v12
// is never fed to this again.
function dropRedundantInvertedTheme(layout) {
  return (layout || []).map((it) => {
    const s = (it && it.settings && typeof it.settings === 'object') ? it.settings : null;
    if (!s || s.theme !== 'inverted') return it;
    const next = { ...s };
    delete next.theme;
    return { ...it, settings: next };
  });
...
// The other half of v12. A config whose every tile says `theme: 'normal'` is
// not fifty independent decisions — it is one decision, taken fifty times
// because there was nowhere else to take it. Lift it to `cfg.faceTheme` so the
// panel switch owns it, and drop the copies.
//
// Deliberately all-or-nothing, and only when the panel has no stated theme of
// its own:
//   - every tile on every screen must say 'normal' (a tile with no theme is
//     an inverted tile, so it disagrees and blocks the lift)
//   - there must be at least one tile, or "all of them" is vacuous
//   - `cfg.faceTheme` must be unset, or it already outranks this
// Under those conditions the lift is render-identical, which is the whole bar
// a migration has to clear.
//
// Idempotent: after one pass no tile carries a theme, so the "all normal" test
// fails on the next — and `cfg.faceTheme` is set by then, which stops it
// earlier anyway.
function liftUniformNormalTheme(cfg, screens) {
  if (cfg && cfg.faceTheme) return { cfg, screens };
  let tiles = 0;
  for (const s of screens) {
    for (const it of (s.layout || [])) {
      tiles++;
      const st = (it && it.settings && typeof it.settings === 'object') ? it.settings : null;
      if (!st || st.theme !== 'normal') return { cfg, screens };
    }
  }
  if (!tiles) return { cfg, screens };
  const stripped = screens.map(s => ({
    ...s,
    layout: (s.layout || []).map((it) => {
      const next = { ...it.settings };
      delete next.theme;
      return { ...it, settings: next };
    })
  }));
  return { cfg: { ...cfg, faceTheme: 'light' }, screens: stripped };
}
...
function migrateConfigToScreens(cfg) {
  if (Array.isArray(cfg.screens) && cfg.screens.length) {
    ...
    // v12: let the panel-wide polarity switch reach tiles that only carry the
    // old default, and lift a uniformly black-on-white config to the panel
    // setting instead of fifty per-tile copies of it.
    if (v < 12) {
      const lifted = liftUniformNormalTheme(cfg, screens);
      cfg = lifted.cfg;
      screens = lifted.screens.map(s => ({ ...s, layout: dropRedundantInvertedTheme(s.layout) }));
    }
