...
//
// Only `'inverted'` is dropped: it is what an unset theme already means, so
// removing it cannot change how anything renders today — it only lets the
// panel switch reach the tile. `'normal'` is kept, because it is the one
// value nobody could have written by accident.
//
...
function migrateConfigToScreens(cfg) {
  if (Array.isArray(cfg.screens) && cfg.screens.length) {
    ...
    // old default. See dropRedundantInvertedTheme.
    if (v < 12) screens = screens.map(s => ({ ...s, layout: dropRedundantInvertedTheme(s.layout) }));
