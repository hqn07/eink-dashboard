...
const { loadSsr } = require('./ssr-shell');
...
async function buildWidgetData(cfg, units, layout) {
  ...
  const wantWeather = ids.has('weather') || ids.has('ai')
  ...
  await Promise.all((layout || []).map(async (item) => {
    ...
    try {
      switch (wid) {
        ...
        case 'weather': {
          // Always populate slot.weather. Empty location resolves to a
          // "NO DATA" stub so the renderer can't fall back to global
          // cfg.weather for this tile.
          //
          // One fetch for all four views: the hero and the outlook come out of
          // the same OpenWeatherMap payload, which is why they were one widget
          // waiting to happen.
          ...
          // Harmless on the `now` views, which never read it.
        ...
        case 'chess':
          ...
          break;
        case 'markets': {
          // A tile with no symbols of its own inherits the shared list, the
          // same rule weather and codeactivity already follow: say a fact
          // once, in Setup, and a new tile asks nothing.
          const own = Array.isArray(eff.symbols) ? eff.symbols.filter(Boolean) : [];
          const symbols = own.length ? own : (homeValue(cfg, 'tickers') || []);
          slot.markets = await fetchMarkets({ ...eff, symbols });
          break;
        }
        case 'sparkline': {
  ...
  if (aiItems.length) {
    ...
    // Phrasings that read better in a prompt than the palette label does.
    // OVERRIDES ONLY — anything missing falls back to the widget's own label
    // from the registry, so this table can never cause a tile to go
    // undescribed. It used to BE the whole map, and it silently rotted through
    // the v8/v9/v10 merges: five keys pointed at widgets that no longer exist
    // (world_clock, aqi, uv, sun, fx) and eleven live widgets had no entry at
    // all, so the model was told about 10 of 22. An `outdoors` tile could sit
    // on the panel and the briefing had no idea it was there.
    const LABEL_OVERRIDES = {
      ...
      tasks: 'a task list',
      moon: 'the moon phase',
      eink_battery: 'the panel battery',
      markets: 'market prices',
      countdown: 'a countdown',
      progress: 'progress bars',
      outdoors: 'sun, UV and air quality',
      daily: 'a quote or word of the day',
      codeactivity: 'GitHub activity',
      sparkline: 'a trend line',
      transit: 'transit arrivals',
      webhook: 'a pushed data readout',
    };
    // Registry labels are the floor: a widget added later is described without
    // anyone remembering to update this file.
    let defs = {};
    try { defs = (await loadSsr()).DEFS || {}; } catch { /* labels only */ }
    const labelFor = (id) => LABEL_OVERRIDES[id]
      || (defs[id] && String(defs[id].label || '').toLowerCase())
      || null;
