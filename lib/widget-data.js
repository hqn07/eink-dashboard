...
async function buildWidgetData(cfg, units, layout) {
  ...
  if (aiItems.length) {
    ...
    const LABELS = {
      ...
      eink_battery: 'panel battery', mac_battery: 'laptop battery',
      mac_nowplaying: 'now playing', stocks: 'stock prices', crypto: 'crypto prices',
    ...
    const otherWidgets = [...new Set((layout || [])
      ...
      .map(id => LABELS[id])
      .filter(Boolean))];
    const merged = { weather, events, aqi, units, city: cfg.city, home: cfg.home,
      otherWidgets, now: Date.now() };
    for (const slot of Object.values(perItem)) {
      ...
      for (const k of ['headlines', 'tasks', 'macNowPlaying']) {
    ...
    await Promise.all(aiItems.map(async (item) => {
      const eff = { ...(item.settings || {}) };
      try {
        const ai = await fetchAi(eff, merged, item.id);
