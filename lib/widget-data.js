...
const { fetchEvents, localEventsToEvents } = require('../widgets/calendar');
...
async function buildWidgetData(cfg, units, layout) {
  ...
  const wantEvents = ids.has('calendar') || usedTokens.has('nextEvent');
    ...
    try {
      switch (wid) {
        ...
        case 'calendar': {
          ...
          // Quick events typed into the tile merge with the feed(s); they
          // also work with no feed configured at all.
          const locals = localEventsToEvents(eff.localEvents);
          if (urls.length || locals.length) {
            const lists = await Promise.all(urls.map(u => fetchEvents(u)));
            slot.events = mergeEvents([...lists.flat(), ...locals]);
          } else {
            slot.events = [];
          }
          break;
