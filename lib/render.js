...
// Past this, a frame whose background re-render has been FAILING gets a
// visible marker composited into it (see markStalePlanes). Age alone is not
// the signal — an old frame nobody asked to refresh is fine. The signal is
// "we tried to refresh this and could not", sustained long enough that it is
// not a blip. Sits inside IMAGE_MAX_STALE_MS so the marker has a window to
// appear in before the hard ceiling forces an inline render.
const IMAGE_STALE_MARK_MS = Math.max(
  IMAGE_CACHE_MS,
  parseInt(process.env.IMAGE_STALE_MARK_MS, 10) || 10 * 60 * 1000
);
...
function renderImage(variant) {
  ...
  const promise = (async () => {
    ...
    const entry = {
      ...
      etagBin: etagOf(bin),
      get3c: async () => {
        // Reuse the mono plane already computed above rather than re-running
        // the whole threshold pipeline (see rgbaToPlanes).
        if (!bin3c) { bin3c = await rgbaToPlanes(rgba, { rawMono, info }); etag3c = etagOf(bin3c); }
  ...
  if (cached) {
    ...
    if (age >= IMAGE_CACHE_MS && !inflightImage.has(key)) {
      renderImage(variant).catch(err => {
        ...
        // Record it ON THE CACHED ENTRY so the next request can tell the
        // difference between "old because nobody asked" and "old because we
        // cannot refresh". A successful render replaces the entry wholesale,
        // so this clears itself — there is no reset path to forget.
        const e = imageCache.get(key);
        if (e && e.at === cached.at) {
          if (!e.failedAt) e.failedAt = Date.now();
...
// Should this frame carry the "you are looking at something old" marker?
// Both conditions must hold: a re-render has actually failed since this entry
// was made, and it has been stale long enough to not be a passing blip.
function imageStaleInfo(entry) {
  if (!entry || !entry.failedAt) return { stale: false, ageMs: 0, failCount: 0 };
  const ageMs = Date.now() - entry.at;
  return {
    stale: ageMs >= IMAGE_STALE_MARK_MS,
    ageMs,
...
module.exports = {
  ...
  IMAGE_CACHE_MS, IMAGE_MAX_STALE_MS, IMAGE_STALE_MARK_MS, imageStaleInfo,
};
