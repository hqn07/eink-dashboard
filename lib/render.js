...
// Panel column-offset compensation. Some panels (e.g. a Waveshare 7.5" B V3
// driven by the GDEY075Z08 class) display the image rotated right by a fixed
// number of columns — the rightmost N px wrap around to the left edge. Set
// PANEL_X_SHIFT_PX to that offset and we pre-rotate the render LEFT by N so it
// lands correctly after the panel's wrap. 0 = off (normal panels).
const PANEL_X_SHIFT = Math.max(0, Math.min(SCREEN_W - 1,
  parseInt(process.env.PANEL_X_SHIFT_PX, 10) || 0));

// Rotate an 800x480 PNG left by n columns, wrapping the leftmost n px around
// to the right edge. Inverse of the panel's right-wrap, so the two cancel.
async function shiftLeftWrap(pngBuf, n) {
  if (!n) return pngBuf;
  const leftStrip = await sharp(pngBuf).extract({ left: 0, top: 0, width: n, height: SCREEN_H }).png().toBuffer();
  const mainPart  = await sharp(pngBuf).extract({ left: n, top: 0, width: SCREEN_W - n, height: SCREEN_H }).png().toBuffer();
  return sharp({ create: { width: SCREEN_W, height: SCREEN_H, channels: 3, background: '#ffffff' } })
    .composite([
      { input: mainPart, left: 0, top: 0 },
      { input: leftStrip, left: SCREEN_W - n, top: 0 }
    ])
    .png().toBuffer();
}
...
function renderImage(variant) {
  ...
  const promise = (async () => {
    let rgba = await renderDashboardPng(variant);
    if (PANEL_X_SHIFT) rgba = await shiftLeftWrap(rgba, PANEL_X_SHIFT);
...
// Always-on hosts proactively render the active screen on an interval so the
// device wake hits a warm cache instead of paying for a cold render. Disable
// with PRERENDER=0; tune cadence with PRERENDER_INTERVAL_MS.
