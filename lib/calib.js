...
function makeTarget() {
  ...
  // --- Diagonal sweeps through both planes ---
  // The single most informative mark on the target. Each line advances a
  // fixed number of columns per row, so it is straight by construction: any
  // horizontal displacement of the data shows up as a visible KINK at the
  // exact row it happens, and the number of kinks counts how many times it
  // happens. Black and red carry interleaved families, so the two planes can
  // be compared directly — a kink in red with black still straight localises
  // the fault to the red plane and to one row.
  const SLOPE = SCREEN_W / SCREEN_H;   // 800/480 — one full sweep top to bottom
  for (let y = 0; y < SCREEN_H; y++) {
    const adv = Math.round(y * SLOPE);
    for (let f = 0; f < 4; f++) {
      const xr = (f * 200 + adv) % SCREEN_W;
      const xb = (f * 200 + 100 + adv) % SCREEN_W;
      for (let t = 0; t < 3; t++) {
        plot(red, (xr + t) % SCREEN_W, y);
        plot(black, (xb + t) % SCREEN_W, y);
      }
    }
  }
