...
const { allowInlineScript } = require('./csp');
...
const AUTOFIT_SCRIPT = (() => {
  const src = fs.readFileSync(path.join(__dirname, '..', 'control-src', 'autofit.js'), 'utf8')
    .replace(/^export\s+/gm, '');
  // The body is hashed for CSP, so build it separately from the tags — the
  // browser hashes only what sits between them.
  const body = `(function(){\n${src}\n`
    + `var __run=function(){requestAnimationFrame(function(){runAutofit(document);window.__autofitDone=true;});};`
    + `if(document.fonts&&document.fonts.ready){document.fonts.ready.then(__run);}else{__run();}})();`;
  return `<script>${allowInlineScript(body)}</script>`;
})();
...
function buildPageBodyHtml({ payload, ssr, mode }) {
  ...
  // Panel-wide polarity (cfg.faceTheme, default dark). Tiles resolve it in
  // cellClasses; the grid container needs it too so uncovered area and card
  // gaps match the tiles instead of glaring white.
  const faceTheme = (cfg && cfg.faceTheme) || 'dark';
  const bodyThemeCls = ssr.bodyThemeClass ? ssr.bodyThemeClass(faceTheme) : '';
  const bodyClass = `body body-grid${cardStyle === 'cards' ? ' body-cards' : ''}${bodyThemeCls ? ' ' + bodyThemeCls : ''}`;
  ...
  if (mode === 'matrix') {
    ...
    for (const id of Object.keys(defs)) {
      ...
      //
      // Variants are the OUTER loop because a variant may declare its own
      // size ladder (weather's forecast views do). Iterating the def's sizes
      // once and reusing them for every variant would render the outlook at
      // the hero's 8x12 — a shape that view never takes on a real panel, so
      // a diff there would mean nothing.
      const variantNames = def.variants ? Object.keys(def.variants) : [null];
      for (const vn of variantNames) {
        const sizes = ssr.sizeSpec(def, vn).sizes || {};
        for (const key of Object.keys(sizes)) {
          ...
          <div style="border:2px solid #000;background:#fff">
            ...
            <div class="cell cell-${escapeHtmlServer(id)} ${ssr.cellClasses(itemCtx.settings, faceTheme).join(' ')}" style="width:${cellWidth}px;height:${cellHeight}px;margin:0">${inner}</div>
          </div>`;
        }
      }
    }
  ...
  if ((mode === 'dev' || mode === 'preview') && devWidgetId) {
    ...
    const extraClasses = ssr.cellClasses(item.settings, faceTheme).join(' ');
  ...
  if (payload.beam && payload.beam.text) {
    ...
    try {
      stamp = new Intl.DateTimeFormat('en-US', {
        timeZone: homeValue(cfg, 'timezone') || 'UTC', hour: 'numeric', minute: '2-digit'
      }).format(sentAt);
  ...
  const nowM = localMinutesNow(homeValue(cfg, 'timezone') || 'UTC');
  const visible = layout.filter(it => withinVisibility(it.visibility, nowM));
  const joins = ssr.tileJoins ? ssr.tileJoins(visible, { cols: GRID_COLS, rows: GRID_ROWS }) : {};
  const cells = [];
  for (const item of layout) {
    ...
    const sw = ssr.scaleWrap(item.settings);
    const inner = `${sw.open}${innerRaw}${sw.close}`;
    const classes = ssr.tileCellClasses(item, GRID_COLS, GRID_ROWS, faceTheme,
  ...
  // Dividers are drawn once, for the page, from the layout — not by each
  // cell's own border. That is what lets two tiles in the same zone share an
  // edge with no rule between them. Card mode keeps its per-card borders:
  // there the border IS the card, and the gaps mean there are no seams.
  const ruled = cardStyle !== 'cards' && cells.length;
  const rulesHtml = ruled && ssr.pageRulesHtml
    ? ssr.pageRulesHtml(visible, { cols: GRID_COLS, rows: GRID_ROWS })
    : '';
  const bodyInner = cells.length
    ? rulesHtml + cells.join('')
    : `<div class="empty terminal-empty" style="grid-column:1 / span ${GRID_COLS};grid-row:1 / span ${GRID_ROWS}">&gt; NO_WIDGETS_ENABLED</div>`;

  const mainClass = `${bodyClass}${ruled ? ' body-ruled' : ''}`;
  return `<div class="page" id="page" style="grid-template-rows:0px minmax(0, 1fr) 0px"><div class="hdr-stub"></div><main class="${mainClass}" style="grid-template-columns:repeat(${GRID_COLS}, minmax(0, 1fr));grid-template-rows:repeat(${GRID_ROWS}, minmax(0, 1fr))">${bodyInner}</main><div class="ftr-stub"></div></div>`;
}
